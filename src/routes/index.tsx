  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="pb-24">
        <div className="mx-auto max-w-[1400px] px-5 lg:px-10">
          <div className="grid auto-rows-[220px] grid-cols-2 gap-4 lg:grid-cols-4">
            {portfolio.map((card) => (
              <a key={card.label} href={card.href} className={`group relative overflow-hidden rounded-sm ${card.span}`}>
                <img src={card.img} alt={card.label} loading="lazy" className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
      ...
      <section className="relative h-[55vh] min-h-[380px] overflow-hidden">
        <img src={bannerLaunch} alt="A new vessel entering the water at launch" loading="lazy" className="absolute inset-0 h-full w-full object-cover" />
      ...
      <section id="contact" className="grid lg:grid-cols-2">
        <div className="flex flex-col justify-center bg-primary px-6 py-20 text-primary-foreground sm:px-16">
          ...
          <a href="/vacancies" className="mt-8 inline-flex w-fit items-center gap-2 rounded-full bg-accent px-7 py-4 text-sm font-semibold text-accent-foreground transition-transform hover:scale-[1.03]">
            View vacancies <ArrowRight className="h-4 w-4" />
          </a>
        </div>
        <div className="relative min-h-[340px]">
          <img src={careerCta} alt="A diverse team of maritime professionals" loading="lazy" width={1000} height={1000} className="absolute inset-0 h-full w-full object-cover" />
        </div>
      </section>
      ...
      <section className="bg-primary py-24 text-primary-foreground">
        <div className="mx-auto max-w-4xl px-5 text-center">
          ...
          <a href="/about" className="mt-10 inline-flex items-center gap-2 border-2 border-white/70 px-7 py-4 text-sm font-semibold transition-colors hover:bg-primary-foreground hover:text-primary">
            ...
            {news.map((n) => (
              <a key={n.title} href="#" className="group">
                <div className="relative aspect-[4/3] overflow-hidden rounded-sm">
                  <img src={n.img} alt={n.title} loading="lazy" className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
                </div>
                <p className="mt-5 text-xs font-semibold uppercase tracking-wider text-muted-foreground">{n.date}</p>
                <h3 className="mt-2 text-xl font-bold text-primary transition-colors group-hover:text-accent">{n.title}</h3>
              </a>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[1400px] px-5 lg:px-10">
          ...
          <div className="grid grid-cols-3 gap-6 sm:grid-cols-4 lg:grid-cols-6">
            {[client1, client2, client3, client4, client5, client6, client7, client8, client9, client10, client11, client12].map((logo, i) => (
              <div key={i} className="flex items-center justify-center rounded-sm bg-white p-5 shadow-sm transition-shadow duration-300 hover:shadow-md" style={{ aspectRatio: "1 / 1" }}>
                <img src={logo} alt={`Client ${i + 1}`} className="h-full w-full object-contain" style={{ maxHeight: "80px" }} />
              </div>
            ))}
          </div>
        </div>
